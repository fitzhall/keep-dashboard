-- Create templates table for Phase 3 (FIXED VERSION)
...
DROP POLICY IF EXISTS "Users can view their own downloads" ON public.template_downloads;
DROP POLICY IF EXISTS "Users can track their downloads" ON public.template_downloads;
...
SELECT 
  'Templates table' as table_name,
  COUNT(*) as row_count 
FROM public.templates
UNION ALL
SELECT 
  'Template downloads table' as table_name,
  COUNT(*) as row_count 
FROM public.template_downloads;

-- 9. Create a simpler view without potential type issues
CREATE OR REPLACE VIEW public.template_stats AS
SELECT 
  t.id,
  t.title,
  t.category,
  (SELECT COUNT(DISTINCT user_id) FROM public.template_downloads WHERE template_id = t.id) as unique_downloads,
  (SELECT COUNT(*) FROM public.template_downloads WHERE template_id = t.id) as total_downloads
